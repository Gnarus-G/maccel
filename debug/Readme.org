...
* Why?
  ...
  + Since this is presents a limited functionality, the USB consortium allows a device to declare =USB Descriptors= of the data it will send, when switching modes away from =Boot protocol=.
